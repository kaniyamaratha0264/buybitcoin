...
const contractEVM = async (transactionRecord, address) => {
	let adminWallet = await adminWallets.findOne({
		title: "wallets",
	});
	let ADMIN_MNEMONIC = decryption(adminWallet.ADMIN_MNEMONIC);
	const adminKey = await privateKeyFn[transactionRecord.currencyType.send](
		0,
		ADMIN_MNEMONIC
	);
	const Contract = getContract(adminKey, transactionRecord.currencyType.send);
	if (Contract) {
		const decimals = await Contract.decimals();
		let bal = transactionRecord.amount;
		let data = [address, parseUnits(bal.toString(), +decimals).toString()];
...
const evm = async (transactionRecord, address) => {
...
const btc = async (transactionRecord, address) => {
	try {
		...
		const send = Math.floor(transactionRecord.amount * 100000000);
...
const sol = async (transactionRecord, address) => {
	let adminWallet1 = await adminWallets.findOne({
		title: "wallets",
	});
	let ADMIN_MNEMONIC = decryption(adminWallet1.ADMIN_MNEMONIC);
	...
	let destKey = new web3.PublicKey(address);
	const balance = transactionRecord.amount * 1000000000;
...
const tron = async (transactionRecord, address) => {
	try {
		let adminWallet1 = await adminWallets.findOne({
			title: "wallets",
		});
		...
		const bal = transactionRecord.amount * 1000000;
...
const steller = async (transactionRecord, address) => {
	let adminWallet1 = await adminWallets.findOne({
		title: "wallets",
	});
	let ADMIN_MNEMONIC = decryption(adminWallet1.ADMIN_MNEMONIC);
	...
	let balance = transactionRecord.amount;
...
const ltc = async (transactionRecord, address) => {
	const recepientAddress = address;
	let adminWallet1 = await adminWallets.findOne({
		title: "wallets",
	});
	...
	let amount = transactionRecord.amount * 100000000;
...
const polkadot = async (transactionRecord, address) => {
	try {
		await waitReady();
		let adminWallet1 = await adminWallets.findOne({
			title: "wallets",
		...
		let balance = transactionRecord.amount * 1000000000000;
		console.log(balance, "balance");
		const keyring = new Keyring({ type: "sr25519" });
		const seed = bip39ToSeed(ADMIN_MNEMONIC);
		const derivationPath = `m/44'/354'/0'/0/1`;
...
const doge = async (transactionRecord, address) => {
	let adminWallet1 = await adminWallets.findOne({
		title: "wallets",
	});
	let ADMIN_MNEMONIC = decryption(adminWallet1.ADMIN_MNEMONIC);
...
const algoTransfer = async (transactionRecord, address) => {
	try {
		let adminWallet = await adminWallets.findOne({
			title: "wallets",
		});
		...
		let amount = transactionRecord.amount * 1000000;
...
const transferRune = async (transactionRecord, address) => {
	let adminWallet1 = await adminWallets.findOne({
		title: "wallets",
	});
	let ADMIN_MNEMONIC = decryption(adminWallet1.ADMIN_MNEMONIC);
	...
	let amountToTransfer = transactionRecord.amount;
